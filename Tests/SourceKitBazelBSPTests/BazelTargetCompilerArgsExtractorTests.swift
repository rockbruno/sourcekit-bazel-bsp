...
struct BazelTargetCompilerArgsExtractorTests {
    ...
    func extractsAndProcessesCompilerArguments_complexRealWorldSwiftExample() throws {
        ...
        let expectedAQuery =
            "bazel --output_base=/private/var/tmp/_bazel_user/hash123-aquery aquery \"mnemonic('ObjcCompile|SwiftCompile', filter(//HelloWorld:HelloWorldLib, deps(//HelloWorld:HelloWorldLib_ios_skbsp)))\" --noinclude_artifacts --noinclude_aspects --output proto"
    ...
    func extractsAndProcessesCompilerArguments_complexRealWorldObjCExample() throws {
        let (extractor, mockRunner, mockRootUri) = Self.makeMockExtractor()
        let expectedAQuery =
            "bazel --output_base=/private/var/tmp/_bazel_user/hash123-aquery aquery \"mnemonic('ObjcCompile|SwiftCompile', filter(//HelloWorld:TodoObjCSupport, deps(//HelloWorld:TodoObjCSupport_ios_skbsp)))\" --noinclude_artifacts --noinclude_aspects --output proto"
    ...
    func missingSwiftModuleReturnsNil() throws {
        let (extractor, mockRunner, mockRootUri) = Self.makeMockExtractor()
        let expectedAQuery =
            "bazel --output_base=/private/var/tmp/_bazel_user/hash123-aquery aquery \"mnemonic('ObjcCompile|SwiftCompile', filter(//HelloWorld:SomethingElseLib, deps(//HelloWorld:SomethingElseLib_ios_skbsp)))\" --noinclude_artifacts --noinclude_aspects --output proto"
        mockRunner.setResponse(for: expectedAQuery, cwd: mockRootUri, response: exampleAqueryOutput)
        let expectedSdkRoot =
            "/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator.sdk"
        mockRunner.setResponse(
